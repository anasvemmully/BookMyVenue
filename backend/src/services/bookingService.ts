...
export class BookingService {
  private getOwnerBookingsCacheKey(ownerId: string): string {
    return `owner:bookings:${ownerId}`;
  }

  private getUserBookingsCacheKey(userId: string): string {
    return `user:bookings:${userId}`;
  }

  async getUserBookings(userId: string) {
    const cacheKey = this.getUserBookingsCacheKey(userId);
    ...
    await cacheService.set(cacheKey, bookings, 600);
    return bookings;
  }

  async getOwnerBookings(ownerId: string) {
    const cacheKey = this.getOwnerBookingsCacheKey(ownerId);
    const cached = await cacheService.get<unknown[]>(cacheKey);
    if (cached) return cached;
  ...
  async createBooking(data: {
    ...
    // Enforce cleanUpBufferMinutes between sequential bookings on same venue
    ...
    for (const existing of existingBookings) {
      ...
      // Check cleanup buffer constraint
    ...
    if (data.ticketTierId) {
      ...
      // Fallback base slot capacity verify
    ...
    const taxAmount = subtotal.mul(0.18); // 18% Tax
    ...
    await cacheService.delete(this.getUserBookingsCacheKey(data.userId));
    await cacheService.delete(this.getOwnerBookingsCacheKey(venue.ownerId));

    return booking;
  }

  async updateBookingStatus(bookingId: string, ownerId: string, status: "CONFIRMED" | "CANCELLED") {
    ...
    // If cancelled/declined, refund/release booked slot capacity
    if (status === "CANCELLED") {
      // Find ticketTierId
    ...
    await cacheService.delete(this.getUserBookingsCacheKey(booking.userId));
    await cacheService.delete(this.getOwnerBookingsCacheKey(ownerId));
  ...
  async receivePayment(
    ...
    // Partial/Advance Payment Validation check on Venue
    ...
    await cacheService.delete(this.getUserBookingsCacheKey(booking.userId));
    await cacheService.delete(this.getOwnerBookingsCacheKey(booking.venue.ownerId));
